...
class ControlClient {
  ...
  setupParameterEnvelopeControls(paramName, suffix, precision) {
    ...
    staticCheckbox.addEventListener("change", () => {
      ...
      const startSlider = document.querySelector(
        `[data-param="${paramName}"][data-type="start"]`,
      );
      const endSlider = document.querySelector(
        `[data-param="${paramName}"][data-type="end"]`,
      );
      ...
      if (isStatic) {
        ...
        if (startMinHandle) {
          this.pendingMusicalState[paramName] = parseFloat(
            startMinHandle.value,
          );
        }
      } else {
        ...
        const envTypeRadio = document.querySelector(
          `input[name="${paramName}-env-type"]:checked`,
        );
    ...
    intensitySlider.addEventListener("input", (e) => {
      ...
      if (
        ...
        typeof this.pendingMusicalState[paramName] === "object"
      ) {
    ...
    document.querySelectorAll(`input[name="${paramName}-env-type"]`).forEach(
      (radio) => {
  ...
  setupDualRangeSlider(paramName, valueType, precision) {
    ...
    const container = document.querySelector(
      `[data-param="${paramName}"][data-type="${valueType}"]`,
    );
    if (!container) return;
    ...
    minHandle.addEventListener("input", (e) => {
      ...
      if (isStatic) {
        ...
        const staticCheckbox = document.getElementById(`${paramName}-static`);
        if (
          !staticCheckbox.checked && container.dataset.mode === "static" &&
          typeof this.pendingMusicalState[paramName] === "object"
        ) {
          if (valueType === "start") {
  ...
  updateDualSliderDisplay(paramName, valueType, precision) {
    ...
    if (isStatic || minVal === maxVal) {
      // Show single value
      display.textContent = precision === 0
        ? minVal.toString()
        : minVal.toFixed(precision);
    } else {
      // Show range
      const minText = precision === 0
        ? minVal.toString()
        : minVal.toFixed(precision);
      const maxText = precision === 0
        ? maxVal.toString()
  ...
  toggleSliderMode(paramName, valueType, precision) {
    ...
    if (newMode === "static") {
      ...
      const maxHandle = container.querySelector(".range-max");
      const avgValue =
        (parseFloat(minHandle.value) + parseFloat(maxHandle.value)) / 2;
      ...
      minHandle.value = Math.max(
        parseFloat(minHandle.min),
        currentVal - range / 2,
      );
      maxHandle.value = Math.min(
        parseFloat(minHandle.max),
        currentVal + range / 2,
      );
